require(xts)
across = read.table('across.csv') 
dates = as.Date(rownames(across), '%Y-%m-%d')
across = xts(across, order.by=dates)
all_sectors = xts(rowSums(across), order.by=dates); # , as.Date(rownames(across)))
...
make_chart <- function(symbol) {
  ...
  if (symbol == 'All.Sectors'){
    ...
    plot.xts(cumprod(1+across[,symbol])-1, main=symbol)
  }
